...
import { getLeaveById, updateLeaveById } from 'apiSdk/leaves';
...
function LeaveEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<LeaveInterface>(
    () => (id ? `/leaves/${id}` : null),
    () => getLeaveById(id),
  );
  ...
  const handleSubmit = async (values: LeaveInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateLeaveById(id, values);
      mutate(updated);
      resetForm();
      router.push('/leaves');
    } catch (error: any) {
