...
import { getJobVacancies } from 'apiSdk/job-vacancies';
import { JobVacancyInterface } from 'interfaces/job-vacancy';
import { getEmployees } from 'apiSdk/employees';
import { EmployeeInterface } from 'interfaces/employee';
import { getLeaves } from 'apiSdk/leaves';
import { LeaveInterface } from 'interfaces/leave';

import { JobVacancyListPage as ShowCaseList } from 'pages/job-vacancies';

function DashboardPage() {
  const { session } = useSession();

  const jobVacancyStat = useSWR<PaginatedInterface<JobVacancyInterface>>(
    () => `/job-vacancies`,
    () => getJobVacancies({ limit: 1, offset: 0 }),
  );
  ...
  const leaveStat = useSWR<PaginatedInterface<LeaveInterface>>(
    () => `/leaves`,
    () => getLeaves({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Job Vacancies',
      result: jobVacancyStat?.data?.totalCount,
      isLoading: jobVacancyStat.isLoading,
    ...
    {
      title: 'Leaves',
      result: leaveStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`job-vacancies`} passHref legacyBehavior>
                  <Button
