...
import { getJobVacancyById } from 'apiSdk/job-vacancies';
import { JobVacancyInterface } from 'interfaces/job-vacancy';

function JobVacancyViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<JobVacancyInterface>(
    () => (id ? `/job-vacancies/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('job_vacancy', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/job-vacancies/edit/${id}`} passHref legacyBehavior>
                    <Button
